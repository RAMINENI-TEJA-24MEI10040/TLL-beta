...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('http://localhost:5000/api/scans').then(r=>r.json()),
      fetch('http://localhost:5000/api/alerts').then(r=>r.json()),
      fetch('http://localhost:5000/api/endpoints').then(r=>r.json()),
      fetch('http://localhost:5000/api/endpoints-spec').then(r=>r.json())
    ]).then(([dash, scans, alerts, endpoints, spec]) => {
      if(dash.SCAN_DATA) {
        SCAN_DATA = dash.SCAN_DATA;
        UPTIME_DATA = dash.UPTIME_DATA;
        LOAD_DATA = dash.LOAD_DATA;
        RADAR_DATA = dash.RADAR_DATA;
        PIE_THREATS = dash.PIE_THREATS;
        CHANGES = dash.CHANGES;
        REPOS = dash.REPOS;
      }
      if(scans.length) SCAN_HISTORY = scans;
      if(alerts.length) ALERTS = alerts;
      if(endpoints.length) ENDPOINTS = endpoints;
      if(spec.length) MOCK_ENDPOINTS_SPEC = spec;
      setForceRender(Date.now());
    }).catch(e => console.log('Backend not running, using static data.'));
  }, []);
